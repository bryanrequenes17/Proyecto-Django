...
def principalCuenta(request):
    ...
    print(usuario.get_all_permissions())
    if usuario.has_perm('modelo.add_cliente'):
        listaCuentas = Cuenta.objects.all().filter(estado=True)
        context = {
            'clientes': listaCuentas,
            'title': "Cuentas",
            'mensaje': "Modulo Cuentas Activas"
        }
...
def buscarC(request):
    usuario=request.user
    if usuario.has_perm('modelo.add_cliente'):
        numero = request.GET['txt_buscar1']
        ...
        return render(request, 'clientes/ver_transaccion.html')
...
def buscarCuenta(request):
    ...
    if usuario.has_perm('modelo.view_cliente'):
        ...
        listaCuentas= Cuenta.objects.filter(numero =dni)
        context = {
            'clientes': listaCuentas,
            'title': "Cuentas",
            'mensaje': "Modulo Cuentas",
